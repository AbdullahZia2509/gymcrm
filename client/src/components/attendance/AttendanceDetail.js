...
const AttendanceDetail = () => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      setAlert(err.response?.data?.msg || 'Error deleting attendance record', 'error');
            ...
            <form onSubmit={handleSubmit}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth required error={!!errors.attendanceType}>
                ...
                {attendance.attendanceType === 'class' && (
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth required={attendance.attendanceType === 'class'} error={!!errors.classSession}>
                      <InputLabel id="class-session-label">Class Session</InputLabel>
                      <Select
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={6}>
                      <TextField
                        type="date"
                        label="Check-In Date *"
                        value={attendance.checkInTime ? new Date(attendance.checkInTime).toISOString().split('T')[0] : ''}
                        onChange={(e) => {
                          try {
                    ...
                    <Grid item xs={6}>
                      <TextField
                        ...
                        value={attendance.checkInTime ? 
                          `${new Date(attendance.checkInTime).getHours().toString().padStart(2, '0')}:${new Date(attendance.checkInTime).getMinutes().toString().padStart(2, '0')}` : 
                          ''}
                        onChange={(e) => {
                          try {
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={6}>
                      <TextField
                        ...
                        value={attendance.checkOutTime ? new Date(attendance.checkOutTime).toISOString().split('T')[0] : ''}
                        onChange={(e) => {
                          try {
                            const datePart = e.target.value;
                            if (!datePart) {
                    ...
                    <Grid item xs={6}>
                      <TextField
                        ...
                        value={attendance.checkOutTime ? 
                          `${new Date(attendance.checkOutTime).getHours().toString().padStart(2, '0')}:${new Date(attendance.checkOutTime).getMinutes().toString().padStart(2, '0')}` : 
                          ''}
                        onChange={(e) => {
                          try {
                            const timePart = e.target.value;
